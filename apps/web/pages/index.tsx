    ...
    <div style={{ fontFamily: "Inter, system-ui, sans-serif", maxWidth: 900, margin: "48px auto" }}>
      ...
      <div style={{ display: "flex", gap: 16 }}>
        <div style={{ flex: 1, padding: 16, border: "1px solid #eee", borderRadius: 8 }}>
          <h3>Sign in (JWT)</h3>
          <button onClick={()=>{
            axios.post((process.env.NEXT_PUBLIC_API_BASE || "http://localhost:9100") + "/api/login", { name: "demo" }, { withCredentials: true })
              .then(()=> window.location.reload());
          }}>Sign in as demo</button>
          <button style={{ marginLeft: 8 }} onClick={()=>{
            axios.post((process.env.NEXT_PUBLIC_API_BASE || "http://localhost:9100") + "/api/logout", {}, { withCredentials: true })
              .then(()=> window.location.reload());
          }}>Sign out</button>
